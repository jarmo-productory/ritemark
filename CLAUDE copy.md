...
- **Branch**: `release`
- **Domain**: `rm.productory.ai`
- **Purpose**: Live production site for end users
- **Deployment**: Manual promotion from `main` after staging validation

**Staging Environment:**
- **Branch**: `main`
- **Domain**: `ritemark.netlify.app`
...
# Create PR to main → merges to staging automatically

# Production Promotion (after smoke tests on staging)
git checkout release
git merge main
git push origin release
# Deploys to rm.productory.ai automatically

# Hotfix Flow (ALWAYS through main first)
git checkout -b hotfix/critical-bug main
# ... fix bug ...
git push origin hotfix/critical-bug
# PR to main → test on staging → promote to release
```
...
2. ✅ Automatic deployment to `ritemark.netlify.app` complete
3. ✅ Smoke tests passed on staging:
   - Authentication works
   - Editor loads and functions
   - Drive sync operations work
   - No console errors
4. ✅ Monitor staging for client-side errors (see monitoring section below)
5. ✅ Team approval (if applicable)

**Promotion Commands:**
```bash
# Promote staging to production
git checkout release
git merge main --no-ff -m "chore: promote main to production"
git push origin release
...
**Rollback Strategy (if production breaks):**
```bash
...
**Both environments use the same configuration** - no environment-specific feature flags needed:
- Same Netlify Functions
- Same build process
- Same environment variables (client ID, etc.)

**🚨 CRITICAL: Google OAuth Redirect URI Configuration**

The OAuth redirect URI is **hardcoded** in `src/components/WelcomeScreen.tsx`:
...
**⚠️ PRODUCTION DOMAIN ISSUE:**

Currently, the redirect URI points to `ritemark.netlify.app` (staging), NOT `rm.productory.ai` (production). This means:

1. **Users on `rm.productory.ai` will be redirected to `ritemark.netlify.app` after OAuth**
2. This creates confusion and breaks the user experience
3. **MUST UPDATE CODE** before first production deployment

**Required Changes Before Production Launch:**

**Option 1: Dynamic Redirect Based on Current Domain (RECOMMENDED)**

Update `WelcomeScreen.tsx` to detect current domain:

```typescript
// Detect if we're on production or staging
const isProduction = window.location.hostname === 'rm.productory.ai'
const isStaging = window.location.hostname === 'ritemark.netlify.app'

const fixedRedirectUri = isLocalDev
  ? 'http://localhost:8888/.netlify/functions/auth-callback'
  : isProduction
    ? 'https://rm.productory.ai/.netlify/functions/auth-callback'
    : 'https://ritemark.netlify.app/.netlify/functions/auth-callback'
```

**Option 2: Environment Variable (Alternative)**

Set `VITE_OAUTH_REDIRECT_URI` in Netlify site settings and use it in code.

**Google Cloud Console Updates Required:**

Add **BOTH** redirect URIs to authorized redirect URIs list:
1. `https://ritemark.netlify.app/.netlify/functions/auth-callback` (staging)
2. `https://rm.productory.ai/.netlify/functions/auth-callback` (production)
3. Keep `http://localhost:8888/.netlify/functions/auth-callback` (local dev)

**Authorization JavaScript Origins (for Google Picker):**
1. `https://ritemark.netlify.app` (staging)
2. `https://rm.productory.ai` (production)
3. `http://localhost:5173` (local dev)

**DNS Configuration:**
- `rm.productory.ai` → Netlify site for `release` branch
- `ritemark.netlify.app` → Netlify site for `main` branch
...
# Production deployment monitoring (after promoting to release)
git push origin release && timeout 300 netlify logs:function log-client-error
```
...
### **🚨 MANDATORY: DRY VIOLATION PREVENTION (AI Guardrails)**
**CRITICAL: AI agents MUST follow these rules when generating code:**

#### Rule 1: ALWAYS Check for Similar Existing Code
**Before creating ANY new service/utility/manager:**

```bash
# Search for similar functionality first
grep -r "class.*Manager" src/services/  # Find existing managers
grep -r "export.*function.*fetch" src/  # Find existing fetch utilities
find src/ -name "*Auth*" -o -name "*Token*"  # Find auth-related code
```

**If similar code exists:**
1. **USE the existing code** - Don't create a duplicate
2. **Extend existing code** - Add features to current implementation
3. **Refactor if needed** - Improve existing code, don't duplicate

**NEVER create parallel implementations** (e.g., `tokenManager` vs `tokenManagerEncrypted`)

#### Rule 2: When Replacing Existing Code - ATOMIC MIGRATIONS ONLY

**WRONG (caused Sprint 26 bug):**
- Sprint 19: Create `tokenManagerEncrypted` (new exists alongside old)
- Sprint 20: Migrate some files (partial migration)
- Sprint 26: Race condition from two parallel systems

**CORRECT (atomic migration):**
```typescript
// Step 1: Create new implementation with @deprecated marker on OLD
/** @deprecated Use tokenManagerEncrypted instead - REMOVE BY SPRINT XX */
export const tokenManager = { /* ... */ }

// Step 2: Migrate ALL usages in SAME sprint
// - Search: grep -r "tokenManager\." src/
// - Replace ALL imports: tokenManager → tokenManagerEncrypted
// - Update ALL method calls

// Step 3: Delete OLD file immediately after migration
// - No partial states
// - No "migrate later" TODOs
```

#### Rule 3: Self-Audit After Code Generation

**After generating ANY service/manager/utility, AI MUST run:**

```bash
# Check if you just created a duplicate
basename <new-file>.ts | xargs -I {} find src/ -name "*{}*" -type f

# Example: Created TokenManagerNew.ts
# Search: find src/ -name "*TokenManager*"
# Result: TokenManager.ts, TokenManagerEncrypted.ts, TokenManagerNew.ts ← 🚨 DUPLICATION!
```

**If multiple similar files found:**
1. **STOP immediately**
2. **Report to user**: "Found existing similar code at X, should I extend it instead?"
3. **Wait for user decision**

#### Rule 4: Detection Patterns for AI Agents

**Red flags that indicate potential duplication:**

```typescript
// 🚨 Similar names with version suffixes
tokenManager.ts + tokenManagerEncrypted.ts
authService.ts + authServiceV2.ts
driveClient.ts + driveClientNew.ts

// 🚨 Multiple implementations of same concept
services/auth/google.ts + services/auth/oauth.ts + services/auth/googleAuth.ts

// 🚨 TODO migration comments
// TODO: Migrate to new auth system
// TODO: Remove old token manager

// 🚨 Conditional imports based on feature flags
import { oldAuth } from './old'
import { newAuth } from './new'
const auth = useNewAuth ? newAuth : oldAuth  // ← PARALLEL SYSTEMS!
```

#### Why This Matters (Sprint 26 Case Study)

**The Bug:**
- `useTokenValidator` imported OLD `tokenManager` (sessionStorage)
- Token refresh happened in NEW `tokenManagerEncrypted` (memory)
- Validator checked stale expiry → logged out after successful refresh

**Root Cause:**
- Incomplete migration left two parallel token systems
- AI picked up OLD pattern from existing code
- No guardrails to detect duplication

**Prevention:**
- AI agents MUST search for existing implementations before creating new
- Migrations MUST be atomic (all-or-nothing in same sprint)
- AI MUST self-audit for duplications after code generation

**NEVER create "V2" or "New" versions - Migrate atomically or don't migrate at all!**
...
### **⚠️ CRITICAL INCIDENT: Sprint 19 Token Migration (async/await Bug)**

**LESSON: NEVER modify async/await without reading method signatures and testing in browser!**

**What Went Wrong:**
1. Migrated from `tokenManager` to `tokenManagerEncrypted`
2. Removed `await` keywords assuming methods were synchronous
3. **CRITICAL MISTAKE**: Never checked that `getAccessToken()` was actually `async`
4. Result: Google Picker received `Promise<string>` instead of `string` → 403 error
5. Claimed "done" after only running `npm run type-check` and `curl localhost:5173`
6. **VIOLATED MANDATORY BROWSER VALIDATION** - never opened browser to test

**The Bug:**
```typescript
// ❌ WRONG (returns Promise object, not string)
const token = tokenManagerEncrypted.getAccessToken()  // Missing await!

// ✅ CORRECT (method is async, must use await)
async getAccessToken(): Promise<string | null> {  // ← IT'S ASYNC!
  // Can trigger token refresh, so MUST be async
}
const token = await tokenManagerEncrypted.getAccessToken()
```

**Secondary Bug - Memory Restoration:**
```typescript
// ❌ Session restore didn't restore tokens to memory
if (storedUser && storedTokens) {
  setUser(JSON.parse(storedUser))  // Restored user profile
  // BUT: Never restored access token to tokenManagerEncrypted memory!
  // Memory-only tokens lost on page reload
}

// ✅ MUST restore memory-only data explicitly
if (storedUser && storedTokens) {
  setUser(JSON.parse(storedUser))
  await tokenManagerEncrypted.storeTokens(tokenData)  // ← RESTORE MEMORY!
}
```

**MANDATORY PRE-FLIGHT CHECKLIST (Before Touching async/await):**

```typescript
// 1. READ THE METHOD SIGNATURE FIRST - Don't assume, VERIFY!
async getAccessToken(): Promise<string | null>  // ← Check: Async? Promise?

// 2. UNDERSTAND WHY IT'S ASYNC
//    - Does it do I/O? (IndexedDB, fetch, setTimeout)
//    - Can it trigger other async operations? (token refresh)
//    - Can it fail in a way that needs await?

// 3. ONLY THEN DECIDE ON await
const token = await tokenManagerEncrypted.getAccessToken()

// 4. ADD DEBUG LOGS for critical token/auth flows
console.log('[Component] Token retrieved:', token ? `${token.substring(0, 20)}...` : 'null')
```

**MANDATORY POST-CHANGE VALIDATION (After Every File Edit):**

```bash
# THE GOLDEN RULE: "TYPE → BROWSER → FEATURE TEST"
# DO NOT SKIP ANY STEP!

1. npm run type-check              # ✅ Catches type errors
2. Open localhost:5173 in browser  # ✅ Catches import/runtime errors
3. Test the actual feature         # ✅ Catches logic errors (Promise issues)
4. Check browser console logs      # ✅ Catches hidden errors

# ❌ NEVER claim "done" after only steps 1-2!
# ❌ TypeScript compilation passing ≠ feature working!
```

**Memory-Only Data Restoration Pattern:**

When data is stored in-memory for security (like access tokens), you MUST restore it explicitly:

```typescript
// ✅ Pattern for session restoration with memory-only data
if (storedUser && storedTokens) {
  // 1. Restore React state
  setUser(JSON.parse(storedUser))

  // 2. Restore memory-only data (THIS IS CRITICAL!)
  await tokenManagerEncrypted.storeTokens(tokenData)
...
**Why This Happened:**
1. ❌ Assumed synchronous based on "reads from memory" without checking signature
2. ❌ Batch-changed 3 files (App.tsx, usePicker.ts, useDriveSync.ts) without incremental testing
3. ❌ Only ran `npm run type-check` - skipped browser validation
4. ❌ Never tested Google Picker feature after changes
5. ❌ Violated MANDATORY VALIDATION protocol from CLAUDE.md

**Prevention Rules:**
1. **NEVER** remove/add `await` without reading the actual method definition
2. **ALWAYS** test in browser after each file change (incremental testing)
3. **ALWAYS** test the specific feature you modified (Google Picker, Drive save, etc.)
4. **ALWAYS** add debug logs when touching token/auth flows
5. **REMEMBER**: Memory-only data (tokens, keys) requires explicit restoration!

**Impact:** Google Picker 403 error, "Open from Drive" completely broken until fixed
...
### **🔐 OAUTH AUTHENTICATION PATTERNS (Sprint 20):**

**CRITICAL ARCHITECTURAL DECISION: OAuth Callback Handling Location**

**ABSOLUTE RULE: ALL OAuth callbacks MUST be handled in App.tsx (top-level route component) ONLY.**

**Anti-Pattern Discovered (Sprint 20):**
- Duplicate OAuth callback handling in both App.tsx (new) and WelcomeScreen.tsx (old)
- Both components processed same callback simultaneously
- Caused race conditions, duplicate token storage, and ugly error dialogs
- Root cause: Incremental migration without cleanup (added new code, never removed old)

**Why App.tsx is the ONLY Correct Location:**

1. **Always Mounts First**
   - App.tsx always renders for `/app` route
   - Guaranteed to see URL params before child components
   - No race conditions

2. **Route-Level Responsibility**
   - OAuth redirect URI points to `/app` route
   - Route handler should process route-level concerns
   - Matches architectural pattern (route → component hierarchy)

3. **Single Source of Truth**
   - Only ONE component processes callbacks
   - Prevents duplicate processing
   - Clear ownership of responsibility

4. **Access to App-Wide State**
   - Can update AuthContext directly
   - Can control which screen renders
   - Can manage authentication lifecycle

**Why NOT Child Components (WelcomeScreen, Editor, etc.):**

1. **Conditional Mounting**
   - Child components may not mount during callback
   - Cannot guarantee callback processing
   - Creates race conditions

2. **Component-Level Responsibility**
   - Child components are presentational
   - Should only handle UI and user interactions
   - Should NOT handle route-level logic

3. **Separation of Concerns**
   - OAuth initiation (button click) ≠ OAuth callback processing (URL params)
   - Button click in WelcomeScreen ✅
   - URL param processing in App.tsx ✅

**Component Responsibilities:**

```typescript
// ✅ App.tsx (Top-Level Route Component)
useEffect(() => {
  // OAuth callback processing (ONLY here)
  const params = new URLSearchParams(window.location.search)
  const accessToken = params.get('access_token')
  if (accessToken) {
    // Process tokens, store user data, reload
  }
}, [])

// ✅ WelcomeScreen.tsx (Presentational Component)
const handleSignIn = () => {
  // OAuth initiation (button click handler)
  window.location.href = backendOAuthUrl
}

// ❌ WelcomeScreen.tsx (NEVER DO THIS)
useEffect(() => {
  // NEVER handle OAuth callbacks in child components!
  const params = new URLSearchParams(window.location.search)
  // This creates duplicate processing and race conditions
}, [])
```

**Incremental Migration Checklist:**

When changing authentication flows, ALWAYS:

1. ✅ Identify which component should handle callbacks (usually top-level route)
2. ✅ Add new callback handling code
3. ✅ Test new implementation thoroughly
4. ✅ **Search codebase for duplicate callback handling** (grep for `access_token`, `window.location.search`)
5. ✅ **Remove old callback code** (don't leave orphaned code!)
6. ✅ Update documentation (ADRs, CLAUDE.md)
7. ✅ Code review for duplicates
8. ✅ Update tests
...
- [ ] Is there existing OAuth callback handling elsewhere?
- [ ] Which component should own this responsibility?
- [ ] Am I duplicating functionality?
- [ ] Can I delete old code?
- [ ] Does only ONE component handle callbacks?
- [ ] Are responsibilities clearly separated?
- [ ] Did I update architecture docs?

**Reference:**
- See `/docs/architecture/ADR-003-oauth-callback-handling.md` for full architectural decision record
- Sprint 19: Browser-only OAuth in WelcomeScreen
- Sprint 20: Backend OAuth migration to App.tsx
- Sprint 20 Bug: Duplicate callback handling causing "Authentication failed" alert
